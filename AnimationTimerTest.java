...
public class AnimationTimerTest extends Application
{
	...
	private static final int		NUM_BALLS	= 2000;
	...
	public void start(Stage primaryStage)
	{
		...
		ballContainer.addEventHandler(MouseEvent.MOUSE_CLICKED, new EventHandler<MouseEvent>()
		{
			...
			public void handle(MouseEvent event)
			{
				...
				{
					balls.clear();
					createBalls(NUM_BALLS, MIN_RADIUS, MAX_RADIUS, MIN_SPEED, MAX_SPEED, ballContainer.getWidth() / 2,
		...
		createBalls(NUM_BALLS, MIN_RADIUS, MAX_RADIUS, MIN_SPEED, MAX_SPEED, 400, 800);
	...
	private void updateWorld(long elapsedTime)
	{
		...
		for (Thing b : balls)
		{
			...
			b.getView().setFill(Color.hsb(250-(b.getXVelocity()*b.getXVelocity()+b.getYVelocity()*b.getYVelocity())*255/(300*200*2), 1, 1));
	...
	private void checkCollisions(double maxX, double maxY)
	{
		for (ListIterator<Thing> slowIt = balls.listIterator(); slowIt.hasNext();)
		{
			...
			for (ListIterator<Thing> fastIt = balls.listIterator(slowIt.nextIndex()); fastIt.hasNext();)
			{
	...
	private void bounce(final Thing b1, final Thing b2, final double deltaX, final double deltaY)
	{
		...
		final double u1PerpX = xVelocity1 - u1 * unitContactX; // Components of ball 1 velocity in direction
																// perpendicular
		final double u1PerpY = yVelocity1/2 - u1 * unitContactY; // to contact vector. This doesn't change with collision
			...
			double initialX, double initialY)
	{
		...
		for (int i = 0; i < numBalls; i++)
		{
			...
			Thing ball = new Thing(initialX, initialY, radius, speed * cos(angle), speed * sin(angle), mass);
